...
check_args() {
	...
	if [ -n "$server_addr" ] && { ! check_dns_name "$server_addr" && ! check_ip "$server_addr"; }; then
		exiterr "Invalid server address. Must be a fully qualified domain name (FQDN) or an IPv6 address."
	fi
...
Note: Make sure this DNS name '$1'
      resolves to the IPv6 address of this server.
EOF
}
...
find_public_ip() {
	ip_url1="http://ipv6.icanhazip.com"
...
check_nat_ip() {
	...
	if check_pvt_ip "$ip"; then
		...
		if ! check_ip "$get_public_ip"; then
			if [ "$auto" = 0 ]; then
				echo
				echo "This server is behind NAT. What is the public IPv6 address?"
				read -rp "Public IPv6 address: " public_ip
...
update_sysctl() {
	...
	echo 'net.ipv4.ip_forward=0' > "$conf_fwd"
